...
axios.interceptors.response.use(response => {
  ...
  let list = response.data; // 根据实际数据结构取得返回的token
  if (list.hasOwnProperty("token")) {
    localStorage.setItem('token', list.token);
    axios.defaults.headers.Authorization = 'ali' + localStorage.getItem('token');
  }
  // 这里如果是需要接口认证时可以添加token认证
  return response;
}, err => {
...
axios.defaults.headers = {
  ...
  "Authorization": 'ali' + localStorage.getItem('token') // token
  // 这里主要是认证类型
}
